...
import org.apache.flink.runtime.iterative.concurrent.SSPClockHolder;
 ...
 * the initial input and establish a {@link org.apache.flink.runtime.iterative.concurrent.BlockingBackChannel} to the iteration's tail. After successfully processing
 ...
 * until it receives an {@link org.apache.flink.runtime.iterative.event.AllWorkersDoneEvent} from the sync, which signals that all other heads have also finished
 ...
 * will send a {@link org.apache.flink.runtime.iterative.event.TerminationEvent} to all it's connected tasks, signaling them to shutdown.
 * <p/>
 ...
 *            same as {@code X}
 */
public class SSPIterationHeadPactTask<X, Y, S extends Function, OT> extends AbstractIterativePactTask<S, OT> {
...
//	private RuntimeAggregatorRegistry aggregatorRegistry;
	...
	/**
	 * the iteration head prepares the backchannel: it allocates memory, instantiates a {@link org.apache.flink.runtime.iterative.concurrent.BlockingBackChannel} and
	 * hands it to the iteration tail via a {@link org.apache.flink.runtime.iterative.concurrent.Broker} singleton
	 */
...
//				barrier.waitForOtherWorkers();
	...
	private void sendEventToSync(WorkerClockEvent event) throws IOException, InterruptedException {
		if (log.isInfoEnabled()) {
			log.info(formatLogString("sending " + WorkerClockEvent.class.getSimpleName() + " to sync"));
		}
